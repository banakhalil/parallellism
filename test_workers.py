...
def get_total_system_ram_mb():
    try:
        ...
        # المرور على كافة (Child Processes) وجمع ذاكرتها
...
def run_with_workers_internal(chunk_size=5000):
    ...
    with ProcessPoolExecutor(max_workers=4) as process_executor:
        futures = [process_executor.submit(
            process_worker_pure, chunk) for chunk in chunks]
...
# خيط مراقبة عالي التردد (كل 50 ملي ثانية) لاقتناص أعلى قفزة (Peak RAM) تستهلكها العملية الأم والعمليات الفرعية معاً
    ...
    try:
        ...
        # ضمان إيقاف خيط مراقبة الرام بأمان بعد انتهاء التنفيذ لمنع أي تسريب للذاكرة
...
if __name__ == '__main__':
    ...
    print("========================================================")
    print(
        f" Waiting to execute Workers Audit automatically at {TARGET_HOUR:02d}:{TARGET_MINUTE:02d} (24h format)")
    print(" Keep this Terminal window open...")
    print("========================================================\n")

    scheduler.add_job(trigger_workers, 'cron',
