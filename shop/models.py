...
class Product(models.Model):
    ...
    # Optimistic locking token incremented on every successful update.
...
class Order(models.Model):
    ...
    # to prevent duplicate orders on retries/double-submit (create_order method).
...
class DailyAuditLog(models.Model):
    audit_date = models.DateTimeField(
        auto_now_add=True, verbose_name="Audit Date & Time")
    method_used = models.CharField(
        max_length=50, verbose_name="Processing Method")
    total_orders_processed = models.IntegerField(
        verbose_name="Total Orders Processed")
    total_sales_amount = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Total Sales Amount")
    ...
    status = models.CharField(
        max_length=20, default="Success", verbose_name="Audit Status")
